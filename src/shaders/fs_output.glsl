...
const float minIndicatorSpeed = 1e-5;
...
void main(void) {
  ...
  vec2 indicatorVel = texture(uFluidData[0], indicatorUV).xy;
  ...
  bool shadeIndicator = uDrawIndicatorLines && (indicatorSpeed > minIndicatorSpeed) && (indicatorLineDistanceField < 7e-4);
  ...
  shadeIndicator = shadeIndicator || (uDrawIndicatorArrows && (indicatorSpeed > minIndicatorSpeed) && (indicatorArrowDistanceField < 7e-4));
  ...
  outColor = shadeIndicator ? indicatorCol : soluteBlend + (shadeWallOutline + shadeWallStripe) * wallCol;
}
